...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Verificar si la contraseña es diferente a la actual y no está vacía
    if (!empty($contrasena_nueva) && $contrasena_nueva != $contrasena_hash) {
        // Hash de la nueva contraseña
        $contrasena_hash_nueva = password_hash($contrasena_nueva, PASSWORD_DEFAULT);

        // Preparar la consulta para actualizar los datos en la base de datos
        $sql_update = "UPDATE usuarios SET nombre = ?, apellido = ?, Correo_electronico = ?, contrasena = ? WHERE id = ?";
        $stmt = $conn->prepare($sql_update);
        $stmt->bind_param("ssssi", $nombre_nuevo, $apellido_nuevo, $correo_nuevo, $contrasena_hash_nueva, $usuario_id);
    } else {
        // Preparar la consulta para actualizar los datos en la base de datos sin cambiar la contraseña
        $sql_update = "UPDATE usuarios SET nombre = ?, apellido = ?, Correo_electronico = ? WHERE id = ?";
        $stmt = $conn->prepare($sql_update);
        $stmt->bind_param("sssi", $nombre_nuevo, $apellido_nuevo, $correo_nuevo, $usuario_id);
    }

    // Ejecutar la consulta
    if ($stmt->execute()) {
        // Redirigir a la misma página para evitar el reenvío del formulario
        header("Location: modificar.php?guardado=true");
        exit();
    } else {
...
<body id="page-top">
                ...
                <div class="container-fluid">
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            <h2 class="m-0 font-weight-bold text-primary">Perfil</h2>
                        </div>
                        <div class="card-header py-3">
                        ...
                        <div class="card-body">
                            ...
                            <br>
                            <a href="dashboard.php">Volver a Dashboard</a>
                        </div>
                    </div>
                </div>
                <!-- /.container-fluid -->
    ...
    <!-- Mensaje de guardado -->
    <?php if (isset($_GET['guardado']) && $_GET['guardado'] == 'true'): ?>
